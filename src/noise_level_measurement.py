...
CHANNELS = int(p.get_device_info_by_index(
    INPUT_DEVICE_INDEX)["maxInputChannels"])
SAMPLING_RATE = int(p.get_device_info_by_index(
    INPUT_DEVICE_INDEX)["defaultSampleRate"])
RECORD_SECONDS = 1
...
def main():
    while True:
        ...
        for i in range(0, int(SAMPLING_RATE / CHUNK * RECORD_SECONDS)):
            ...
            elm = np.frombuffer(elm, dtype="int16") / \
        ...
        print("elapsed_time:{:.3f}[sec], DB:{:.3f}[db]".format(
            elapsed_time, db))
